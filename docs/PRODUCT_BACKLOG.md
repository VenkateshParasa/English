...
- **Total is 480 points** (§16), of which 277 are done. At a realistic 8–12 points a week of evenings,
  the **remaining 203** is a **4–5 month** effort. Sprints 0–1 hold most of the audit's value and are
...
Three briefing errors, recorded so they are not repeated from this document.
...
| **US-138** | `FR-DATA-4` export does not cover recordings. With blobs in IndexedDB, `CON-3`'s "export is the only backup" is now false for them. **Sharpened by `US-136`: the gap has stopped being theoretical** — learners now actually have recordings, on two surfaces, and this row is the whole named residue of `US-605` | `FR-DATA-4` | 3 | M |
...
| ✅ **US-194** | **`js/core/blobstore.js` suite written — 0% → 98.78% statements / 92.33% branch / 99.29% funcs, 211 tests.** Needed a hand-built fake IndexedDB: spec key ordering, `IDBKeyRange.bound`, `autoIncrement`, compound-index `getAll`, abort-rollback, and a settable byte budget that throws `QuotaExceededError`. Found **five defects** (`US-216`–`US-220`) plus `US-221`/`US-222`; the single uncovered line is the dead code `US-221` names — **done 2026-09-12**. **The suite has since grown with the fixes it prompted: 241 tests, 99.45% statements, 93.28% branch, 99.33% funcs, 100% lines** (`US-227`, `US-228`) | `NFR-16`, `FR-DATA-6` | 3 | M |
...
**Sprint 1 total: 250 points across 137 stories — 89 done (180 points), 48 remaining (70 points).**
The remaining items are
almost all hygiene, copy or wiring surfaced by later work; `US-110` (crossword crediting a blank
grid) is the last learner-facing honesty defect and waits on `OQ-7`. Eight rows closed on
2026-09-12 — `US-136`, `US-185`, `US-227`–`US-230`, and the two new content rows `US-236`/`US-237`,
which were **filed and closed in the same wave** — and five were filed open by the same work
(`US-238`–`US-242`), plus amendments to `US-192` and `US-194`. `US-136` is the one that matters
beyond this table: it is the caller `blobstore.js` never had, and it closed `US-404` and `US-605`
in two other sprints without a row of its own changing sprint. The table shrank and grew at about the
same rate again, which is what a wave spent finishing a defect class looks like — every one of the
five new rows was found *because* the previous ones were fixed.
...
| **US-503** | Everyday points 9–16 (**split from an 8**). **1 of 8 authored** — present perfect vs past simple (9, `US-178`). `grammarLessons.everyday` holds **two** points, but the second is `register.js` (`US-237`), which registers into the `everyday` tier while carrying `syllabusNumber: 25` and is therefore **not** one of points 9–16 — see `US-242`. Tier content and syllabus coverage have come apart here, and this row counts the syllabus | `FR-GRM-1` | 5 | M |
...
`US-225` (7), `US-226` (5), `US-236` (8), `US-237` (25, outside the syllabus — `US-242`). `US-502` and
`US-503` are the **rollups** for the same work, so `US-502` ticks now that its eighth point exists,
while `US-503` stays open at 1 of 8; their 5 points each are counted once, here.
Grammar content is now **10 points authored: 8 `foundation`, 2 `everyday`** — `foundation` is complete
...
| ✅ **US-605** | Recording archive, last N per prompt, IndexedDB. **Now done — the module has a caller (`US-136`) and the archive works end to end.** `js/core/blobstore.js` (`US-206`) implements per-prompt retention of the pinned baseline plus the two most recent, quota handling and graceful degradation, and remains the best-tested file in the repo (241 tests, 99.45% statements, 93.28% branch, **100% lines**), with all seven defects its suite found (`US-216`–`US-222`) plus `US-227`/`US-228` closed. `RecordingArchive` in `app.js` calls it from the listening/read-aloud path and the pronunciation self-comparison, keyed on a **stable content `promptId`** rather than an index. **Residue, named and not counted here: the archive is not inside `FR-DATA-4`'s export.** That is `US-138` — a `M`, 3-point Sprint 1 story against a *different* requirement — so leaving this row open would count one missing export twice. This follows `US-403`, which was credited when its own criterion passed and had its residue filed as `US-235` rather than held inside the row. `FR-SPK-6` and `FR-DATA-6`, the two requirements this row names, are both met — **done 2026-09-12** | `FR-SPK-6`, `FR-DATA-6` | 5 | S |
...
self-report, listening's `✓ I said it`, which replaced the `alert()`-and-stop
dead end a refused microphone used to hit (`US-703`), and — the newest and the one that mattered most
to get right — `US-404`'s A→B→A self-comparison, which is appended *after* the existing skip pair and
tells a device with no microphone in one sentence that the task was never the recording. Like
`FR-A11Y-5` in §13a it is a per-story
check, not work in itself, so it cannot close while `US-601`, `US-602`, `US-606` and `US-607` are
unbuilt.

**Sprint 6 total: 36 points — 5 of 36 done** (before splitting US-602). **The sprint opened on
2026-09-12**, and not by a Sprint 6 story: `US-136` gave `blobstore.js` its caller, which closed
`US-605`. The done column had been held at 0 on purpose while the module was finished, fully tested
and uncalled; it moves now because the archive is reachable, not because the tests grew. `US-138`
(recordings inside the export) is the named residue and is counted in Sprint 1.
...
| `US-605` | `US-206` (IndexedDB) ✅ **satisfied**, and `US-136` (the caller) ✅ **satisfied 2026-09-12** — row closed; `US-138` carries the export residue |
...
| `US-237`'s syllabus number | **`US-242`** — a `CURRICULUM.md` §3 decision, not code |
...
|---|---|---|---|---|
| 0 | Unblock | 6 | 6 | 6 |
| 1 | Honesty | 247 | 177 | 253 |
| 2 | Data integrity | 19 | 19 | 272 |
| 3 | Extensibility | 22 | 22 | 294 |
| 4 | Pronunciation | 33 | 20 | 327 |
| 5 | Grammar | 33 | 15 | 360 |
| 6 | Speaking | 36 | 5 | 396 |
| 7 | Listening & vocabulary | 40 | 8 | 436 |
| 8 | Session & platform | 44 | 5 | 480 |

`US-501` is listed in both the Sprint 1 and Sprint 5 tables — one story, one done record, ticked
in both. Its 3 points are counted under Sprint 5 here, so the Sprint 1 row reads 247 where its table
sums to 250, and 177 where its table sums to 180. **That 3-point offset is the only difference
between the two, and it is the whole reason to recount from the tables rather than from this one:**
the last two recounts each found a real arithmetic error here — a 5-point gap in the grand
denominator, then 300 against 306 in the MoSCoW split.

**480 points total, of which 277 are done — 58%.** Must-have work is **229 of 311 (74%)**. **`npm test` is green: 9 suites, 1,230 tests, 78.41% statement coverage** (73.85% branch, 80.8% functions, 78.3% lines). **Sprints 0, 2 and 3 are complete.** The wave's keystone is **`US-136`** — `js/core/blobstore.js` finally has a caller. `RecordingArchive` in `app.js` wires it into listening/read-aloud and the pronunciation self-comparison on a `promptId` derived from **content, never an index** (`digest(contentKey(text))` under an `ID_VERSION` namespace, proved stable across an insertion at the front of a tier and across a tier reversal). One 3-point Sprint 1 row closed **`US-605`** in Sprint 6 and **`US-404`** in Sprint 4, which is why two sprints moved without a story of their own being built: Sprint 6 came off 0 for the first time, and Sprint 4's A→B→A ships with no score, no verdict, a watchdog on `onend`, and a test that reads the content files and fails if any authored `feelCheck` ever asks about the sound rather than the feel. `blobstore.js` remains the best-tested file in the repo — **241 tests, 99.45% statements, 93.28% branch, 100% lines** — and its last two open defects closed as **copy and accounting decisions rather than logic changes**: `US-227` keeps `ok: true` and fixes only the claim (`removeForPrompt` names a *state*, `remove(id)` names *a thing*), and `US-228` counts stranded foreign rows instead of repairing them, because repair rewrites `createdAt` to a fabricated date and thereby makes the row the next eviction's first target — repair is deletion with a date on the way. **Grammar `foundation` is complete, 8 of 8** (`US-236`, modals, closing `US-502`), and **`US-237`** (register, T-G9) closed the last dead drill target — **26 live, 0 dead**, against four dead two waves ago. The false-label class is finished: `US-185` (six producers, not the briefed three, pointing in both directions — one label true of all six), `US-229` (one row for a missing helper in a question *and* a negative, because each shape has one producer and two rows of one never rank in a top five) and `US-230` (both labels already true; document the mapping, do not widen into vagueness). What remains is still concentrated in Sprint 1 — 70 points of hygiene, copy and wiring, plus `US-110` (the crossword crediting a blank grid), the last learner-facing honesty defect, which waits on `OQ-7`. At 8–12 points a week of evenings the remaining 203 points are roughly **4–5 months**.

**Five rows were filed open by this wave** (`US-238`–`US-242`), every one of them found because the
row before it was fixed: two copy residues in `blobstore.js` (`US-238`, an empty `clear()` still
reporting a deletion; `US-239`, `remove(id)` deleting a stranded row that `list()` denies exists —
kept deliberately for targeted recovery, now pinned, and the reason `US-228`'s old "only `clear()`
removes them" is no longer quite true), one measurement defect (`US-240`, `strandedBytes` trusting a
row's *claimed* `size`), one authoring affordance (`US-241`, an explicit `promptId` so a typo fix can
keep the recordings `US-136`'s content-keyed derivation would otherwise orphan), and one curriculum
decision (`US-242`, `CURRICULUM.md` §3 Strand B names no register point, so `register.js` took
`syllabusNumber: 25`).

**Four stories remain partly satisfied and deliberately not ticked**, each with the residue named in
its row — `US-409` (no connected-speech or intonation content), `US-503` (1 of 8 everyday points;
`register.js` registers into the tier but carries syllabus 25 and is not one of them), `US-507`
(Telugu notes yes, L1 ordering no), `US-508` (audit done on two points, contradicted by `US-166`).
`US-502` left this list on 2026-09-12 and `US-605` left it from the other direction — it was held at
not-done while its module was finished and uncalled, and **the tick it now has is for a caller, not
for tests**. Its export residue is `US-138`, which is counted once, in Sprint 1, against
`FR-DATA-4` — the requirement `US-605` does not name.
