...
is how it rots. **`npm test` is green — 9 suites, 1230 tests, 0 failures, 78.41% statement coverage**
(73.85% branches, 80.8% functions, 78.3% lines) — every live `js/core` module has a suite. **Sprint 0
is complete, 6 of 6:** `package-lock.json` is committed (`391b1a5`, 200,050 bytes), so a clean
checkout can finally `npm ci`. The machinery is largely built: Grammar and Pronunciation exist as
sections with all authored content browsable, the session sequencer has a UI so a learner can press
one button and be walked through a session, typed `gram:`/`phon:` reviews render, the mistake panel is
on the dashboard with producers in four strands, and **every drill button in the app now opens
something — 26 live targets, 0 dead**, because content registers its own destinations. Two things
changed this wave that had been promised for waves and untrue: **the recording archive has a caller**,
so a recording survives leaving the sentence and `TEACHING_METHODOLOGY.md:83` ("keep the recording")
is met, and **the A→B→A self-comparison exists** — model, the learner, model again, with no score and
no verdict. Grammar content is **10 points of the 24 in the syllabus, and the whole `foundation` tier
is now authored** (8 of 8; the 2 `everyday` points are syllabus 9 and a reported gap, see B12).
What remains is mostly **content, not code**: 14 grammar points unauthored; **no listening
comprehension question — the `questions` field exists and nothing authors or draws it**; and no
free-production surface with a timer and rubric (`FR-SPK-3`). The one thing left in the test story is
CI itself — the workflow's test job is still commented out (B1), so the 1230 tests run only when
someone runs them.
...
- ✅ ~~**`blobstore.js` is the best-tested file in the repo and still has no caller.**~~ **Closed in
  Wave 17 (`US-136`)** — `RecordingArchive` at `app.js:5991-6819` persists every recording and
  `TEACHING_METHODOLOGY.md:83` is met. Left in place because it was the headline of this wave's
  shortfall and the next wave's headline delivery, which is the pattern worth seeing.
- **`lsn.gist` still has no producer.** The listening section asks **no** comprehension question; the
  ...
  (`data.js:970-972`). The field existing is not a surface. **Still true after Wave 17.**
- **`US-110` and `OQ-7` are still open** — the crossword still credits a blank grid, and that is the
  last learner-facing honesty defect in the app. **Still true after Wave 17.**
...
incrementally. The lesson became a decision in §7: a content or test agent must **write a complete
file early and refine it in place**, never hold a whole deliverable until the end of one long run.
⚠️ **That wording was wrong and is amended in §7 as of Wave 17.** Three more agents died the next
wave, all at the *moment of writing* — a 42–70KB file cannot be written in one call — so "write a
complete file first" was an instruction to do the thing that kills the agent. The corrected rule is
terse-but-complete first, then enrich by edits.

---

### 2.11 Wave 17 — the recording is kept, and the learner can hear themselves (2026-09-12)

**Suite state, measured this wave:** `npm test` → **9 suites, 1230 tests, 0 failures**, up from
1136. `npx jest --coverage` → **78.41% statements (2380/3035)**, 73.85% branches, 80.8% functions,
78.3% lines. Point totals are `PRODUCT_BACKLOG.md` §16's arithmetic, not this ledger's.

| # | Item | Evidence |
|---|---|---|
| ✅ | **`US-136` / `US-605` — `blobstore.js` has a caller, so the archive is a feature rather than a module.** For two waves this ledger recorded a fully-tested module nobody called, and `TEACHING_METHODOLOGY.md:83`'s *"keep the recording"* was untrue for that reason. `RecordingArchive` is the seam between the store and the two surfaces that record | `app.js:5991-6819`, a self-contained IIFE between BEGIN/END markers so `__tests__/unit/sections.test.js` can extract and evaluate it against the fake IndexedDB (`app.js` cannot be required — see `__tests__/README.md`). The one call into the store is `s.put(promptId, blob, meta)` at `app.js:6510` |
| ✅ | **`promptId` is stable content identity, never an index.** Derived from the sentence's own words: NFKC-normalised, apostrophes **dropped** so `don't` / `don’t` / `dont` agree, non-alphanumerics collapsed to one space, lowercased, then two 32-bit FNV-1a hashes base-36 | `app.js` `contentKey()` / `promptIdFor()`: `'<kind>:' + ID_VERSION + ':' + digest(contentKey(text))`. **The stability proof ran rather than being asserted** — a sentence inserted at the front of a tier pushes the learner's sentence to index 1 and it keeps the same id and still resolves to the right sentence; reversing a tier permutes ids identically; `rate`, `seconds`, `situation`, `focus`, `notes`, `shadow`, `questions` and `tier` all change without moving it. Pronunciation uses `pair.id` verbatim — already authored stable identity |
| ✅ | **`US-404` — the A→B→A self-comparison is real, and it scores nothing.** Model → the learner → model, both model plays at `PRON_RATE_SLOW` and the learner's clip at its own rate, chained on `onend` with a watchdog | `app.js:9084-9260+`, `pronSpeakSequence()` / `pronPlayLearner()` / `pronRunComparison()`. **Verified by grep: zero `Mistakes.` and zero `SRS.` calls in the whole comparison block** (`app.js:9084-9460`), and the suite pins it — `appendPronunciationComparison()` and `pronRunComparison()` are asserted to contain no `Mistakes.record(`, no `SRS.` and no `correct` assignment (`__tests__/unit/sections.test.js:2766`) |
| ✅ | **The self-check question is the authored *feelable* one, and a test now defends that against the content.** *"Did your top teeth touch your bottom lip?"* — never *"did it sound right?"*, which is unanswerable by exactly the person who needs the answer | `sections.test.js:2745` reads **both** pronunciation content files and fails if any `feelChecks` block contains `sound right` / `sound correct` / `did it sound`. The honest line this project keeps having to defend is now enforced on the content, not only on the code |
| ✅ | **The copy *"This app does not record you, so nothing is played back and nothing is scored"* is gone from live code** — and it went because it stopped being true, not because it was inconvenient | 0 occurrences outside comments; `app.js:9089-9092` records that the note *"was correct and is now gone, because the thing it apologised for exists"*. What replaced it still refuses what it cannot do: the app can play the model and play you back to yourself; judging your voice is the thing it will not attempt |
| ✅ | **Every drill button in the app now opens something: 26 live targets, 0 dead** — against four dead two waves ago and one at the start of this one | `data/grammar/register.js` closed the last of them. Re-verified for this ledger by loading `mistakes.js`, all 9 grammar files and both pronunciation files in one context: **before registration 0 live / 0 dead / all no-claim; after, 0 dead**, `gram.register-indian` → `gram:register` now resolves `true`, and the 6 remaining no-claim rows are the strands nothing registers (collocation, listening, reading) |
| ✅ | **`US-236` — `modals.js` (syllabus 8) is the last missing `foundation` point, which completes the tier** | `data/grammar/modals.js`, wired at `index.html:744`, precached, cache **v19 → v20**. Verified by registration: `foundation` **8 of 8** (syllabus 1–8 all present) |
| ✅ | **`US-237` — `register.js` (T-G9) lands, taking content to 10 points (8 `foundation`, 2 `everyday`)** | `data/grammar/register.js`, `index.html:752`. ⚠️ It took **`syllabusNumber: 25`**, the first free slot, because `CURRICULUM.md §3` Strand B lists 24 points and **none of them is register or audience** — documented as a reported gap rather than squatting on a named point. Needs your decision: **B12** |
| ✅ | **`US-185` closed — `gram.tense-agreement`'s label was false for *everything* routing to it, and now names what is true of all six producers** | `js/core/mistakes.js:770` — *"A verb in the wrong time for the rest of the sentence"*. Id unchanged, so nothing already logged is orphaned |
| ✅ | **`US-229` decided as one row, not two.** *"Where you live?"* and *"I not know"* both reach `gram.auxiliary-omitted`, whose label now names the negative shape as well as the question | `js/core/mistakes.js:578`, `:663`, `:803`. Destination unchanged — `question-formation` is where both are taught |
| ✅ | **`US-230` left split on purpose**, after establishing that both labels are currently true of everything reaching them | `js/core/mistakes.js:434`, `:833`. The comment says outright that nothing in the row changed **and that is the finding** |
| ✅ | **`US-227` — `removeForPrompt` of an unknown prompt keeps returning `ok: true` and only stops *claiming* a deletion** | `js/core/blobstore.js:178-182`, new `MESSAGES.nothingToRemove`: *"There was nothing to delete — this prompt has no saved recordings on this device."* |
| ✅ | **`US-228` — stranded rows are counted and reported, never repaired and never deleted** | `js/core/blobstore.js` `strandedSummary()` (`:419`), `usage()` reports `strandedCount` / `strandedBytes` (`:1554`), `clear()` reports `strandedRemoved` (`:1532`) and is the only thing that frees one (`:1511`) |
| ✅ | **Four incidental defects the archive work found and fixed** | `.recording-status.info` had been written since `US-701` and never styled (now `styles.css:3603`); the **microphone stream could stay hot**, because the old handler stopped tracks only on its own click path (`app.js:6401`); an object URL was created for a 0-byte blob and *"Recording saved!"* printed over it (now guarded at `app.js:7021` / `:9317` and `:7399` / `:9447`); and the Record/Stop buttons were only re-enabled from the Stop click |

**Why `promptId` is the part worth recording properly.** The obvious key is
`state.currentListeningIndex`, and it is the one thing that must not be used. Indices **renumber**:
insert one sentence at the front of a tier and every recording made after it silently belongs to the
next sentence, so the month-one/month-three comparison Strand E.7 calls its strongest motivator would
be comparing two different sentences — and the app would have no way to notice. It would look fine
until months of recordings had accumulated, which is the worst possible failure shape for this
feature. So the id is keyed on **what the learner actually said**. `ID_VERSION` exists so that any
future change to the derivation becomes a **visible new namespace** instead of silent orphaning.
Tier is deliberately not in the key — a sentence promoted from `foundation` to `everyday` is the same
sentence — and the cost is stated rather than hidden: the same sentence authored into two tiers shares
one archive, which for a recording of a learner saying that sentence is the correct answer.
**Editing the sentence itself does move the id, deliberately**, because otherwise the learner's voice
would play against words they never said. The opt-out for a genuine typo fix is filed as `US-241` and
is not built.

**The `register.js` frame, because a lesson about Indian English built as right-word/wrong-word is
both wrong and insulting.** It is a **three-way split**: forms that *cost you the message* (`doubt`,
`revert`, `pass out` — the same word with a different meaning, so the reader is confidently wrong and
never asks); forms that communicate perfectly and *merely mark you as Indian* (`out of station`,
`kindly`, `do the needful` — explicitly not a problem to fix); and forms that are *fine everywhere and
corrected anyway*. Only the first group is worth attention, and the file says so. It also solved the
mode problem: `app.js` renders only gap items, but register wants a "who are you writing this to?"
judgement — so the audience is built into every prompt, and **the same word is graded both ways in
adjacent items.** `doubt` is wrong in one and the *only* right answer in the next, with each item
pointing at the other: what changed was the message, not the vocabulary.

**The `modals.js` decisions.** `could` in a request is taught as **distance, not the past of `can`** —
one idea from which `would`, `might` and *"I was wondering if…"* all follow, instead of a formula
each. `may` is taught as *whose permission is at stake*, explicitly **not** as a politeness rung,
because the ladder is precisely what produces *"May you send me the file?"*. And one item exists to
refuse to grade register as grammar: the bare imperative is told plainly that it **is correct
English**, with the reason it lands badly on an external reader — Telugu marks respect on the verb,
English marks it through indirectness, so the respect was encoded where the reader is not looking.

**Three labels resolved as a set, finishing a class this ledger has worked through since
`gram.copula`.** Two things are worth keeping:

- **A stale briefing, caught by the agent.** I briefed it that `gram.tense-agreement` had three
  producers, all in `be.js`. That was **already wrong when I wrote it** — `past-simple.js` had added
  three more that same day, pointing the opposite way. A label naming either direction would still
  have been false. The new one names what is true of all six.
- **`US-229` is the reverse of the previous wave's call, and the argument matters more than the
  outcome.** Each shape — *"Where you live?"* and *"I not know"* — has exactly one producer, so two
  rows of one producer each, ranked in a top five, means a learner with the habit plausibly sees
  **neither**. One row ranks. Both are the absence of the *same word* from the same cause: Telugu
  borrows no word to ask or to negate, so there is no Telugu word for English `do` and it does not
  come to mind. `US-224` split its row because the label was actually **false** of a case; here both
  would be true, so the deciding factor is **ranking, not honesty**. `US-230` stays split for the
  mirror reason: its two halves are opposites — embedded means too much inversion, direct means too
  little — so a single wording could only say "the inversion is in the wrong place", which is the
  vague label this whole class of fix exists to remove.

**Two blobstore judgements, both decided against the tidier option.**

- **`removeForPrompt` of an unknown prompt still returns `ok: true`.** It names a **state** — this
  prompt keeps nothing — which holds when it returns, where `remove(id)` names a **thing** that either
  existed or did not. Reporting failure would paint an error over a satisfied request and force every
  caller to special-case one failure code as success, which teaches callers to ignore `ok`.
- **Stranded rows — a null `createdAt` written by something other than this module — are counted and
  reported, not repaired.** Repair was rejected as **self-defeating rather than merely impure**: a
  repaired row is unflagged and dated to the epoch, making it the oldest unprotected row, so the very
  next eviction takes it. Repair is deletion with a fabricated date on the way. Deleting fails `BR-7`
  outright, and the likeliest author of such a row is *this module before the last wave's fix*, so the
  likeliest content is the learner's own. Ignoring fails toward **loss**, because the cap is then
  computed over a total that is not the total; counting fails toward a **refusal**, which is the
  direction this file already errs in.

**Process, and the third consecutive wave it has cost time.** Three content agents died having written
nothing. **My earlier diagnosis was wrong** — I had recorded that they read too long before writing,
and both of this wave's deaths were at the *moment of writing*: these content files are 42–70KB, and a
single write that large times out. The instruction I had been giving in §7 ("write a complete file
first, then refine") was itself the cause. The fix is to write a **terse but structurally complete**
file, then enrich section by section with edits. `register.js` was authored that way and succeeded; I
wrote `modals.js` myself the same way after the third death. §7's decision is amended rather than
added to, because the old wording is now known to be harmful.

**A ledger-hygiene finding, and it is the largest correction in this wave.** Checking §3.5 and §5 as
instructed turned up **nine rows that recorded work as not-done which had in fact shipped, some of
them fourteen to sixteen waves earlier**: §6's phase table read `☐` for phases 1–7 and 9; §3.5 item 2
(the `StorageManager` docs) was fixed by W4 in Wave 1; and W4, W5, W6, W8 and W11 in the loose-wins
table were all `☐` against work landed in Waves 1–3. Every one is now corrected against the source,
with the wave and story that closed it. This is the same defect as the outward-facing ones, pointed
inward — and it is worse here than elsewhere, because §5 is described at the top of this file as *"the
list I need answers on"* and the loose-wins table as things you can *"pick any of cold"*, so a reader
triaging them would have spent a sitting on five items that were already done. The cause is
structural: waves have been recorded in §2 and §8 without sweeping the index sections those entries
invalidate. **Every future wave must re-check §6's phase table and the loose-wins table against
source, not only §3.5 and §5.**

**What is still wrong after this wave, stated plainly:**

- **`US-138` is genuinely open: the archive is not in `FR-DATA-4`'s export.** A learner who exports
  their data does not get their recordings. Verified — `js/core/portability.js` contains **zero**
  references to recordings, blobs, audio or `BlobStore`.
- **`US-406` (the duration-comparison hint for epenthesis) is unblocked by the recorder and not
  built**, and a free-production surface with a timer and rubric still does not exist (`FR-SPK-3`).
- **`US-110` / `OQ-7` — the crossword crediting a blank grid is still the last learner-facing honesty
  defect**, and still waits on your decision about the puzzles (B8).
- **The CI test job is still commented out** (B1). Its comment block no longer quotes a rotting test
  count, which is an improvement — see §3.5 item 4.
...
| **Voice recording** via `MediaRecorder` + `getUserMedia` | `app.js` | Capture works — and since Wave 17 (`US-136`) it is **wired to the archive**: `RecordingArchive` (`app.js:5991-6819`) keeps each recording in IndexedDB under a content-derived `promptId`, so it survives leaving the sentence. What is still missing is the **rubric-plus-timer free-production surface** (`FR-SPK-3`) and export of the archive (`US-138`) — not the recorder and no longer the archive. |
...
2. ✅ **Fixed 2026-09-09 (W4), and this entry was stale for sixteen waves.** `docs/FOLDER_STRUCTURE.md`
   and `docs/ERROR_HANDLING_GUIDE.md` used to document `new StorageManager(errorHandler, validator)`
   as the app's architecture, and that wiring exists nowhere. Both snippets are now inside explicit
   caveats — *"⚠️ DOES NOT EXIST IN THE CODEBASE — aspirational design, do not copy as-is"*
   (`FOLDER_STRUCTURE.md:196`, `ERROR_HANDLING_GUIDE.md:192`), with `ERROR_HANDLING_GUIDE.md:185`
   heading the section *"Step 2: Initialize in app.js — ⏭️ never done"*. `FOLDER_STRUCTURE.md` goes
   further and records that every `Validator` member is `static`, so `new Validator()` would produce an
   object with no usable methods. The line numbers this ledger cited (`:123-124` and `:57-58`) had also
   rotted. **The aspirational design is still in the docs, which is correct** — B7 says harvest before
   deleting, and the snippet is the harvest list.
3. 🔁 **An `app.js` comment says listening has no mistake destination and gives a stale reason — still
   half-stale, unchanged this wave, and its line numbers have moved again to `app.js:2391-2397`.** The
   comment reads *"listening — FR-LSN-1: nothing in this build asks a comprehension question, so
   neither `lsn.gist` nor `lsn.detail` has a destination"*, and `lsn.detail` has had a **producer**
   (dictation) since Wave 15. Wave 16 half-repaired it: the comment now records that `US-701` made a
   question *possible* and that an empty array is not a question, which is **true** — nothing authors
   or draws them. What is still stale is the same clause as the last two waves, that `lsn.detail` has
   no destination *because* nothing is asked; the narrow point the function needs (neither category has
   a **drill destination** to open) is right, and the reason is not. One comment, and the citation in
   this ledger has now rotted twice.
4. ✅ **Fixed 2026-09-12 — `.gitignore` no longer claims the workflow runs `npm ci`.** It now says
   ...
   ✅ **And the dated claim that had moved into the workflow is gone too, resolved this wave.**
   `.github/workflows/ci-cd.yml` had quoted *"9 suites, 1046 tests, 77.4% statement coverage"* as the
   reason re-enabling is safe, and the ledger's objection was that a comment quoting a measurement
   rots every wave. The comment now says so itself: *"Deliberately no test count or coverage figure
   here. The first version of this comment quoted '112 unit assertions'; the second quoted '1046 tests,
   77.4%'. Both were stale within a wave. Run `npm test` for the current number."* The job is still
   commented out — that is B1, not an honesty defect.
5. ✅ **Fixed 2026-09-12 (`US-136`) — `TEACHING_METHODOLOGY.md:83` *"Keep the recording. Progress over
   weeks is the reward here, not a score today"* is now true.** `js/core/blobstore.js` has a caller:
   `RecordingArchive` at `app.js:5991-6819` puts every recording into the archive per prompt
   (`s.put(promptId, blob, meta)` at `app.js:6510`), keyed on **content identity** rather than an index,
   and `US-404`'s A→B→A comparison plays a learner's own clip back — including one made weeks ago,
   which is what `CURRICULUM.md:226` (Strand E.7) asks for. The blob no longer dies with the
   navigation. **Two residual limits, neither of which is this claim:** the archive is *not* in
   `FR-DATA-4`'s export (`US-138` — `portability.js` has zero references to it), and the written
   `localStorage`-only constraint has still never been formally amended (B11).
6. 🆕 **`CURRICULUM.md:284` (gap #1) is badly stale and understates the project.** It says grammar
   content is *"3 points authored of the 24 in §3, of which 2 are reachable"* and that *"`be` (no. 1)
   exists as `data/grammar/be.js` and is **not in `index.html` or the precache list**"*. Measured this
   wave: **10 points register**, all 10 are wired and precached, `be.js` is at `index.html:721`, and
   `grammarLessons.everyday` is not empty. This is the rarer direction of drift — a repo claim that is
   untrue *against* the work — but it is the same defect, and gap #1's status should be re-judged.
7. 🆕 **`CURRICULUM.md:295` (gap #12) says *"`topCategories()` is never rendered — there is no
   top-5-in-30-days view"*.** It has been rendered since Wave 13: `renderMistakePanel()` at
   `app.js:2497` reads `Mistakes.topCategories()` (`:2517`) into `#mistakePanel` (`index.html:188`), and
   `updateDashboard` calls it (`app.js:2161`). Gap #12's "Errors are now logged and still not surfaced"
   conclusion is false.
...
|---|---|---|---|
| ✅ **B1** | ~~**Jest is not in `devDependencies`.**~~ **Resolved in two parts.** 2026-09-10: the blocker was never the code but an expired token in `~/.npmrc` pointing npm at a private registry; `jest@30.5.1` + `jest-environment-jsdom` are declared and installed. 2026-09-12: `package-lock.json` is **un-ignored and now committed** — `391b1a5`, 200,050 bytes — closing `US-001`. | `npm test` is green on a clean checkout: **9 suites, 1230 tests, 0 failures, 78.41% statements** (73.85% branches, 80.8% functions, 78.3% lines). The shim-only caveat is gone — every assertion has now run under real Jest, which is what caught the three defects of Wave 14 and the seven of `blobstore.js`. | ⚠️ **One thing is still not done, and it is the one that matters for trust:** the `test` job in `.github/workflows/ci-cd.yml` is **still commented out**, so nothing runs the suites on push. Both reasons the comment block gives for disabling it are now false, and it says so itself: *"the reason it stays off is no longer technical."* Uncommenting the job and restoring `needs: test` on `build` is the remaining step. ✅ The stale test count that used to sit in that comment is **gone as of Wave 17** — it now says to run `npm test` for the current number instead of quoting one (§3.5 item 4). |
| 🔴 **B2** | **Existing `srsData` is substantially noise** — the `correct: 0` bug graded most online vocabulary answers effectively at random. Migrate it, or offer a one-time reset? | Phase 2 rewrites persisted data and needs this answer first. ⚠️ **Overtaken in part:** the CEFR rename and its migration shipped in Wave 6 with `SCHEMA_VERSION` at 2, so this is no longer gating a phase — what is still unanswered is the *policy* question about the noisy history itself. | **Migrate plus backup.** The *set* of words seen is still signal even where the verdicts are not. Add a learner-facing "reset my review history" button rather than deciding for them — ✅ that button now exists (`Portability.resetReviewHistory`, Wave 5), so only the default policy is open. |
...
| ☐ **B11** | **Recording archive vs. the `localStorage`-only constraint.** `CURRICULUM.md` Strand E.7 wants the last N recordings kept per prompt, but audio blobs cannot live in `localStorage` (string-only, ~5–10 MB) — that needs **IndexedDB**, which the stated constraint forbids (§6.y A17). ⚠️ **Never formally answered, and now shipped either way.** `js/core/blobstore.js` was written in Wave 6 on the assumption you would say yes; Wave 16 finished and fully tested it; **Wave 17 gave it a caller and put it in front of learners** (`US-136` / `US-404`). The code has decided this twice over; the written constraint still has not. | 🔽 **The cost has changed shape rather than gone away, so this stops being a 🔴 blocker.** It is no longer "the best-tested module in the repo has no caller" — the archive is live, `TEACHING_METHODOLOGY.md:83` is met, and the month-one/month-three comparison the curriculum calls its strongest motivator is reachable through `US-404`'s A→B→A. What is left is narrower and purely documentary: **`REQUIREMENTS.md`'s constraint says `localStorage` only, and the app now writes learner audio to IndexedDB.** A shipped feature contradicting a stated hard constraint is a docs defect, but it is the kind that makes the next author distrust the whole constraint list. | **Amend the constraint to "client-side storage only; `localStorage` for state, IndexedDB for blobs."** Still no backend, still offline, still no build step — it only widens *which* browser store is allowed. This is now a one-line edit that ratifies what is already running, not a gate on any work. Two real follow-ons, both filed rather than blocked: **`US-138`** (the archive is not in `FR-DATA-4`'s export — verified, `portability.js` has zero references to it — so a learner who exports their data does not get their recordings) and **`US-241`** (an authored `promptId` opt-out, so a typo fix does not orphan an archive). |
| ☐ **B12** | 🆕 **The register/audience point has no home in the syllabus.** `CURRICULUM.md §3` Strand B lists **24 numbered grammar points and not one of them is register or audience** — the only mention of register in that doc is a *vocabulary* field under Strand A (`CURRICULUM.md:79`), and `REQUIREMENTS.md` maps **T-G9** to "Strand A register labels" rather than to a Strand B number. So Wave 17's `data/grammar/register.js` took **`syllabusNumber: 25`**, the first free slot, and says so in its own header rather than squatting on a named point. | The point itself is not in doubt — T-G9 is a named Telugu-L1 interference row, it was the last dead drill target in the app, and it teaches the highest-frequency real-world *writing* problem the target learner has. What is in doubt is **where it belongs**, and the answer changes a published count: either the grammar syllabus is 25 points, or register is a Strand A/E concern that happens to be authored as a grammar file, or §3 gains a numbered point and the existing 24 renumber. Leaving it at 25 undocumented would quietly make every "N of 24" figure in this ledger wrong. | **Add it to `CURRICULUM.md §3` Strand B as a 25th point under `everyday`, and do not renumber 1–24.** Renumbering would invalidate every `syllabusNumber` already authored and every reference in this ledger for no gain. A 25th point is the honest record of the fact that the syllabus was written before this row was understood as teachable content. |
...
|---|---|---|---|
| 0 | Test harness + migration spine | none | ✅ **done and verified under real Jest** — 9 suites, 1230 tests, 0 failures, 78.41% statements. Sprint 0 is 6 of 6. Remaining: uncomment the CI test job (B1) |
| 1 | **Honesty:** word-level speech diff, delete fake IPA, fix the `correct: 0` grading bug | low | ✅ **all three named fixes landed in Waves 1–3** — `US-101`/`US-105` (grading + real distractors), `US-102` (fake IPA gone), `US-103` (LCS word diff, "✓ Perfect!" deleted). The three bullets below are kept as the historical statement of the bugs, **not** as open work |
| 2 | CEFR rename + migration | **highest** | ✅ **shipped in Wave 6** (`US-201`/`US-202`) — `data.js` keys are `foundation`/`everyday`/`confident`, `SCHEMA_VERSION` is 2 with three ordered steps, and exercise ids were migrated so completion history survived. **B2 was never a gate on this** and is still open as a *policy* question about the noisy `srsData` |
| 3 | `SECTIONS` registry refactor | high | ✅ **shipped in Wave 7** (`US-301`), `js/core/sections.js`; extended to the dashboard in Wave 11 (`US-163`). Adding a section went from 23 `app.js` edits to 1 |
| 4 | SRS generalisation (`vocab:` / `gram:` / `phon:` / `coll:` keys) + migration | high | ⏳ **substantially done.** Keys namespaced with zero drift in Wave 6 (`US-302`/`US-303`), the fixed `[1,3,7,16,35]` ladder landed in Wave 12 (`US-131`), typed reviews render in Wave 13 (`US-177`). **`coll:` is declared unrenderable** because `data/collocations.js` does not exist — a producer without a destination |
| 5 | Content file layout + foundation/everyday authoring | low | ⏳ `data/grammar/*` and `data/pronunciation/*` exist and self-register. **Grammar: `foundation` is 8 of 8 complete as of Wave 17; `everyday` is 2**. Vocabulary enrichment (collocations, word families, register fields) is unstarted, and B9's IPA-at-scale question is unanswered |
| 6 | Grammar section — the largest hole in the curriculum | low | ⏳ **the section is real and the syllabus is 10 of 24.** Shipped in Wave 8 (`US-501`) with reason/contrast/retry feedback and `gram:` SRS items. What is left is content, and `register.js` needs a syllabus home (B12) |
| 7 | Pronunciation section — minimal pairs, stress, schwa | low | ⏳ **the section is real.** Discrimination drill with per-pair SRS and an 80% production gate (Wave 10, `US-401`), 8 pair sets / 78 minimal pairs, 36 stress and noticing items browsable (`US-197`), and **`US-404`'s A→B→A self-comparison as of Wave 17**. Still missing: A6 (isolated phonemes) and the prosody sub-syllabi A10–A12, which no free source supplies (§6.y) |
...
| 9 | Dashboard, daily goals, session sequencer | low | ⏳ **built, and further along than this row said for six waves.** The sequencer is `js/core/session.js` (Wave 11, `US-801`) with a UI that renders `omitted`/`shortfall` rather than swallowing them (Wave 12, `US-170`); the dashboard is registry-driven (`US-163`); daily goals work; and the top-5 mistake panel ships (Wave 13, `US-181`). What is left is the success-metric surfaces, which wait on B4/B5 |
| 10 | Hygiene — dead code, service worker, CI | low | ☐ the four never-instantiated `js/core` modules are still loaded on every page (§3.4, `US-809`), and the CI test job is still commented out (B1). Retarget per B7 before deleting |

⚠️ **This table had gone stale by several waves and was corrected in Wave 17.** Phases 1–7 and 9 all
read `☐` while §2 and §8 recorded the work as done, which is exactly the contradiction B3 was kept
visible for. Read it as an index of *where the app is*, and `IMPLEMENTATION_PLAN.md` for what a phase
contains.

**The three Phase 1 bugs, kept as the record of what was wrong — all three are fixed:**

- ✅ `app.js` — options are shuffled but `correct` stays `0`, so the graded answer is whichever option landed at index 0. Affects every curated word where the dictionary API succeeds, which is the default online path. This verdict feeds `SRS.schedule()`. *(The generated-word path was correct — it used `options.indexOf(definition)`.)* Fixed Wave 1, `US-101`/`US-105`.
- ✅ `app.js` — `pronunciation: "/" + word + "/"` produced fake IPA like `/joyful/`, teaching learners that IPA is spelling in slashes. Fixed Wave 2, `US-102`.
- ✅ `app.js` — `transcript.includes(target)` printed **"✓ Perfect!"**. Praise for an uncorrected error is how errors fossilise. Fixed Wave 3, `US-103`.
...
> Still current as of Wave 17: the four never-instantiated modules — `storage.js`, `validator.js`,
...
> `US-701`), "`rate` is never varied" (the speed control shipped, `US-704`), and — as of Wave 17 —
> **"the recording blob is a closure variable discarded on navigation"**: `RecordingArchive`
> (`app.js:5991-6819`) persists it to IndexedDB under a content-derived `promptId` (`US-136`).
...
| V9 | ~~**Grammar content quality** — "Phase 6 is fully unaided" is a claim about my capability, unverifiable by inspection~~ **Half-answered, and this row was stale for nine waves.** The authoring happened: `US-500` wrote the articles point end to end (`docs/GRAMMAR_SAMPLE_REVIEW.md`), and **ten points now register** — 8 `foundation` (the whole tier, syllabus 1–8) and 2 `everyday`. So "can it be written" is answered yes by demonstration, and at tier scale rather than one sample. | What is **still** unverified is the half only you can do: **read one and judge it.** Three good tests, each of which makes a contestable call: `question-formation` declines to mark Indian-English invariant *"isn't it?"* wrong; `prepositions` splits *"discuss about"* (a redundancy the learner can reason out) from *"good in maths"* (arbitrary — the file says to stop reasoning) and refuses to describe them in one sentence; and Wave 17's `register` grades **the same word both ways in adjacent items**, telling the learner plainly that *do the needful* and *out of station* are not problems to fix. That last one is the sharpest test, because a reviewer who disagrees with its premise will disagree with the whole file. | ~1 sitting |
...
| A17 | Recording archive — last N per prompt (E.7) | ✅ **Built, tested and — as of Wave 17 — called.** Audio blobs cannot go in `localStorage` (string-only, ~5–10 MB), so `js/core/blobstore.js` uses **IndexedDB**; 224 tests, 99.05% statements, 100% lines, every filed defect fixed (Wave 16, `US-216`–`US-222`). `RecordingArchive` (`app.js:5991-6819`) is the caller it never had (`US-136`): every recording is `put()` under a **content-derived** `promptId`, the first recording per prompt stays pinned, and `US-404` plays it back against the model. E.7 is met and so is `TEACHING_METHODOLOGY.md:83`. **Two things still open, neither of them the archive:** the written `localStorage`-only constraint has never been amended (B11), and the archive is **not in the export** (`US-138`). |
...
##### What the code did when this was written, and where each one landed

Kept as the record, because §6.z's design was argued against these four and all four are now closed:

- ✅ substring match → **"✓ Perfect!"** — replaced by the LCS word diff (Wave 3, `US-103`).
- ✅ target word chosen at random on every render, so the learner could not retry the sound they just
  failed — the target is now the played sentence (Wave 4, `US-119`/`US-104`).
- ✅ recognition errors all collapsed to one generic toast, so `no-speech` (say something) was
  indistinguishable from `not-allowed` (grant mic permission) — differentiated in Wave 3 (`US-106`).
- ✅ the recording blob was a closure variable discarded on navigation, with **no comparison against
  the model and no archive** — both shipped in Wave 17: the archive (`US-136`) and the A→B→A
  comparison (`US-404`).
...
> ✅ **Built in Wave 17 (`US-404`), and it implements rules 1–5 of the six below.** `app.js:9084`
> onward: model → the learner → model again, both model plays at one rate, the learner's clip at its
> own; the question asked is the authored `feelChecks[0]`; the task is gated behind 80% discrimination
> on the pair; and the archive makes the cross-week comparison real. Rule 6 was already in place
> (`SRS.scheduleItem(..., { selfReported: true })` on "not yet", and the affirmative reaches SRS not at
> all). **Nothing in the comparison block calls `Mistakes.` or `SRS.` — verified, and pinned by the
> suite.** Still not built: the duration comparison below (`US-406`), which the recorder unblocked.
...
| 5 | **Keep recordings and compare across weeks, not within the session** | This is where self-comparison is strongest and the perception problem weakest: month-1 vs month-3 differences are large enough for an untrained ear. `CURRICULUM.md:226-227` already calls this the strongest motivator. ✅ **Shipped in Wave 17** — the archive keeps them (`US-136`) and `pronPlayLearner()` falls back to an archived row when there is no session recording, which is what makes the comparison work *across* sessions rather than only within one. B11 is now a documentation question, not a prerequisite. |
...
| ✅ **W4** | Correct `docs/FOLDER_STRUCTURE.md` and `docs/ERROR_HANDLING_GUIDE.md` — **done 2026-09-09**, and this row sat `☐` for sixteen waves after the work landed | They documented a `new StorageManager(errorHandler, validator)` architecture the app never adopted. Both snippets now carry *"⚠️ DOES NOT EXIST IN THE CODEBASE — aspirational design, do not copy as-is"* and the `ERROR_HANDLING_GUIDE.md` section is headed *"never done"*. Verified this wave; see §3.5 item 2. |
| ✅ **W5** | Call `Migrations.migrateProgress()` from `loadProgress()` — **done 2026-09-09 (Wave 3, `US-003`)**, and this row sat `☐` for fourteen waves after the work landed | `migrateStoredProgress()` (`app.js:410`) takes `backupOnce` first, calls `Migrations.migrateProgress()` (`app.js:428`), persists the result and soft-fails on a missing module — and it was done before Phase 2 needed it, as this row asked. The call site is itself idempotent, not just the module. §3.3's "zero call sites" line is a dated 2026-09-08 observation; see the note under §6.x. |
| ✅ **W6** | Make the Sentences exercise mode deterministic per index instead of `Math.random()` — **done 2026-09-09 (Wave 2, `US-108`)**, and this row sat `☐` for fifteen waves after the work landed | `app.js:4512` is now `exerciseTypes[state.currentSentenceIndex % exerciseTypes.length]`, so a failed item can be retried in the mode it was failed in and the retry `TEACHING_METHODOLOGY.md §2` requires is reachable. |
...
| ✅ **W11** | Differentiate `recognition.onerror` cases — **done 2026-09-09 (Wave 3, `US-106`)**, and this row sat `☐` for fourteen waves after the work landed | The `speechRecognitionErrors` table (`app.js:1657`, read at `:1781`) gives distinct non-blaming messages for `no-speech`, `not-allowed`, `service-not-allowed`, `audio-capture`, `network` and a fallback, so the learner can tell whether the app broke or they did. `aborted` returns early with no toast *and* no `logError`, since it fires on ordinary cancellation. |
...
| ✅ **W8** | Build quiz distractors from **other `vocabularyData` entries'** definitions — **done 2026-09-08 (Wave 1, `US-105`)**, and this row sat `☐` for sixteen waves after the work landed | `getDistractorDefinitions(correctDefinition, count)` (`app.js:1603`) feeds the option list at `app.js:1579`; `"Something different"`, `"Unrelated concept"` and `"Opposite meaning"` have **0 occurrences** in `app.js`. It was fixed alongside the `correct: 0` bug on the same line, exactly as this row asked, and the helper degrades safely — the option list is always 4 and `correct` always valid even with `vocabularyData` deleted. *(Deliberately not from the API's synonyms: options are definitions, not words, and `decide` returns zero antonyms.)* |
...
| 2026-09-12 | ~~**A content or test agent must write a complete file early and refine it in place**~~ — **amended in Wave 17, because this wording was itself the cause of three more deaths.** The rule is now: **write a terse but structurally complete file in one pass, then enrich it section by section with edits.** Never emit a large finished file in a single write. | The original diagnosis was that agents read too long before writing. That was **wrong**. Both of Wave 17's deaths were at the *moment of writing*: these content files are 42–70KB, and a single write that large times out on its own, so "write a complete file first" was an instruction to do the thing that kills the agent. The corrected rule keeps what was right about the old one — never hold a whole deliverable to the end of a long run — and removes what was fatal. `register.js` was authored this way and succeeded; `modals.js` was written the same way by hand after the third death. Recorded as an amendment rather than a new row because the old wording is now known to be harmful. |
| 2026-09-12 | **A recording's `promptId` is stable content identity, derived from the sentence's own words — never an index, and never the tier.** `'<kind>:' + ID_VERSION + ':' + digest(contentKey(text))`, where `contentKey()` NFKC-normalises, drops apostrophes, collapses non-alphanumerics and lowercases. **Editing the sentence deliberately moves the id.** | An index **renumbers**. Insert one sentence at the front of a tier and every recording made after it silently belongs to a different sentence — so the month-one/month-three comparison `CURRICULUM.md:226` calls the strongest motivator would compare two different sentences, and the app would have no way to notice. It would look correct until months of recordings had accumulated, which is the worst available failure shape. `completedExercises` stamps are positional and that is survivable, because a lost tick is a tick; a recording is the learner's own voice. Editing the sentence must move the id for the mirror reason: keeping the old recordings under it plays the learner's voice against words they never said. `ID_VERSION` makes any future change to the derivation a **visible new namespace** instead of silent orphaning, and tier is excluded so a promoted sentence keeps its archive — at the stated cost that one sentence authored into two tiers shares one archive, which for a recording of someone saying that sentence is correct. |
| 2026-09-12 | **The self-comparison plays and never judges.** `US-404` puts the model and the learner's own clip next to each other, asks the **authored feelable** question, and contains no `Mistakes.record` and no `SRS.` call at all. A test reads both pronunciation content files and fails if any authored `feelChecks` entry asks about the **sound**. | "Did it sound right?" is unanswerable by exactly the person who needs the answer — L2 perception is filtered through L1 phonology, so the blind spot sits on the interference points the app exists to fix (§6.aa). Asking about something the learner can **feel** sidesteps it. Guarding it in code only would leave the door open through content: an authored question about the sound puts the unanswerable question back on screen without touching a line of `app.js`, so the guard has to read the content files. And the copy this replaced — *"This app does not record you, so nothing is played back and nothing is scored"* — was removed **because it stopped being true**, not because it was inconvenient; what replaced it still refuses what the app cannot do. |
| 2026-09-12 | **`removeForPrompt()` of an unknown prompt returns `ok: true` and stops claiming a deletion**, rather than reporting failure. | It names a **state** — this prompt keeps nothing — and that state holds when the call returns, where `remove(id)` names a **thing** that either existed or did not. Reporting failure would paint an error over a satisfied request and force every caller to special-case one failure code as success, which teaches callers to ignore `ok` — and a caller that ignores `ok` is how a real deletion failure gets missed. |
| 2026-09-12 | **Stranded rows are counted and reported, never repaired and never deleted.** `usage()` returns `strandedCount`/`strandedBytes`; only `clear()` frees them, and it says how many it took. | Repair is **self-defeating, not merely impure**: a repaired row is unflagged and dated to the epoch, making it the oldest unprotected row, so the very next eviction takes it. Repair is deletion with a fabricated date on the way. Deleting outright fails `BR-7`, and the likeliest author of such a row is *this module before Wave 16's fix*, so the likeliest content is the learner's own voice. Ignoring them fails toward **loss**, because the cap is then computed over a total that is not the total; counting them fails toward a **refusal**, and a refusal is never a loss. |
| 2026-09-12 | **Indian English is taught as a three-way split by *cost to the message*, never as right-word/wrong-word.** Forms that cost you the message (`doubt`, `revert`, `pass out`); forms that communicate perfectly and merely mark you as Indian (`out of station`, `kindly`, `do the needful` — explicitly not a problem to fix); and forms that are fine everywhere and corrected anyway. Only the first group gets attention. | A lesson about Indian English built as a list of wrong words is both wrong and insulting: these are the standard forms of a variety with a billion-odd readers, and most of the list is nineteenth-century British official English that Indian English kept and British English dropped. The forms worth learning are the ones that **fail without a sound** — the reader is confidently wrong and never asks — and there are only about four of them, which is what makes the point teachable at all. Same principle as declining to call invariant *"isn't it?"* an error: a false claim about correctness is the defect this app is being redesigned to remove, whichever direction it points. |
| 2026-09-12 | **Where `app.js` renders only gap items but the content needs a judgement, the judgement goes *into the prompt* — and the same word is graded both ways in adjacent items.** `register.js` builds the audience into every prompt, and `doubt` is wrong in one item and the only right answer in the next, each pointing at the other. | Adding a new exercise mode to `app.js` to carry one content file would be the larger change and the wrong one. More importantly, the adjacent-items trick teaches the actual lesson in a way prose cannot: a learner who sees the same word marked both ways cannot conclude "this word is wrong", only "this depends on who is reading" — which is the whole point. A single-verdict item set would have taught a word list. |
| 2026-09-12 | **`could` in a request is taught as *distance*, not as the past of `can`; `may` is taught as *whose permission is at stake*, not as a politeness rung.** | Distance is one idea from which `would`, `might` and *"I was wondering if…"* all follow, so the learner gets a mechanism instead of four formulas. And the politeness ladder is precisely what produces *"May you send me the file?"* — teaching `may` as "more polite than `can`" causes that error rather than preventing it. `modals.js` also spends one item saying the bare imperative **is correct English** and explaining why it lands badly on an external reader: Telugu marks respect on the verb, English marks it through indirectness, so the respect was encoded where the reader is not looking. Grading register as grammar would be the same false-correctness claim in a different strand. |
| 2026-09-12 | **When two error shapes have one producer each, they get ONE taxonomy row — ranking beats splitting.** `US-229` keeps *"Where you live?"* and *"I not know"* in `gram.auxiliary-omitted`, against `US-224`'s decision one wave earlier to split. | The two calls look contradictory and are not. `US-224` split because the label was **false** of a case, and honesty is not negotiable. Here both labels would be **true**, so the deciding factor is whether a learner ever *sees* the finding: two rows of one producer each, ranked in a top five, means a learner with the habit plausibly sees **neither**. One row ranks. And they are the same fact — Telugu borrows no word to ask or to negate, so there is no Telugu word for English `do` and it does not come to mind. `US-230` stays **split** for the mirror reason: its halves are opposites (embedded = too much inversion, direct = too little), so one wording could only say "the inversion is in the wrong place", which is the vague label this class of fix exists to remove. |
| 2026-09-12 | **A label must be true of every producer that routes to it, and the producer list must be re-derived rather than taken from a brief.** `gram.tense-agreement` now reads *"A verb in the wrong time for the rest of the sentence"*, true of all six. | I briefed the agent that there were three producers, all in `be.js`. That was **already wrong when I wrote it**: `past-simple.js` had added three more that day, pointing the opposite way, so a label naming either direction would still have been false. This is the second wave running in which a briefing of mine was stale and the agent caught it (Wave 16 was the `T-G9`/`T-G6` mix-up). The rule that follows is about method, not about this row: a label change must be justified against a **freshly enumerated** producer list, because the list changes faster than the brief. |
...
| 2026-09-12 | **Wave 17 — the recording is kept, and the learner can hear themselves.** Written up in full at **§2.11**. `npm test` **9 suites / 1230 tests / 0 failures**, coverage 78.18% → **78.41% statements** (73.85% branches, 80.8% functions, 78.3% lines). **`US-136` / `US-605`: `blobstore.js` has a caller**, so the recording archive is a feature rather than a module — `RecordingArchive` (`app.js:5991-6819`, a self-contained IIFE between BEGIN/END markers so the suite can extract and evaluate it) puts every recording into IndexedDB at `app.js:6510`, and `TEACHING_METHODOLOGY.md:83` ("keep the recording") is **true for the first time**. **The part worth recording is `promptId`: stable content identity, never an index.** `'<kind>:' + ID_VERSION + ':' + digest(contentKey(text))`, where `contentKey()` NFKC-normalises, **drops** apostrophes so `don't`/`don’t`/`dont` agree, collapses non-alphanumerics and lowercases — so the id is keyed on *what the learner actually said*. An index renumbers, and month-one recordings would silently start belonging to someone else's sentence, looking fine until months had accumulated. **The stability proof ran rather than being asserted:** a sentence inserted at the front of a tier pushes the learner's to index 1 and it keeps the same id and still resolves; reversing a tier permutes ids identically; every non-text field can change without moving it — **and editing the sentence itself does move it, deliberately**, because otherwise the learner's voice plays against words they never said. `ID_VERSION` makes a future change to the derivation a visible new namespace instead of silent orphaning. **`US-404`: the self-comparison is real** — model → the learner → model, both model plays slowed and the learner's clip at its own rate, chained on `onend` with a **watchdog**, because `onend` has been observed never to fire on some Android WebView voices. **No score, no verdict:** the comparison block contains no `Mistakes.record` and no `SRS.` call at all, verified by grep and pinned by the suite. The question asked is the authored **feelable** one, and a test now reads *both* content files and fails if any authored check ever asks about the **sound** instead of the feel. The copy *"This app does not record you, so nothing is played back and nothing is scored"* is gone from live code — **it went because it stopped being true**, not because it was inconvenient — and what replaced it still refuses what the app cannot do. **Every drill button now opens something: 26 live targets, 0 dead**, against four dead two waves ago and one at the start of this one; `data/grammar/register.js` closed the last. **Two grammar points → 10 (8 `foundation`, 2 `everyday`), which completes the `foundation` tier.** `modals.js` (`US-236`, syllabus 8) teaches `could` as **distance, not the past of `can`** — one idea from which `would`, `might` and *"I was wondering if…"* follow — and `may` as *whose permission is at stake*, explicitly **not** a politeness rung, because the ladder is what produces *"May you send me the file?"*; one item exists to refuse to grade register as grammar, telling the learner the bare imperative **is correct English** and why it lands badly (Telugu marks respect on the verb, English through indirectness, so the respect was encoded where the reader is not looking). `register.js` (`US-237`, T-G9) is framed as a **three-way split by cost to the message** — forms that cost you the message (`doubt`, `revert`, `pass out`, where the reader is confidently wrong and never asks), forms that communicate perfectly and merely **mark you as Indian** (`out of station`, `kindly`, `do the needful` — explicitly not a problem to fix), and forms that are fine everywhere and corrected anyway — because a lesson about Indian English built as right-word/wrong-word is both wrong and insulting. It also solved the mode problem by building the audience into every prompt and **grading the same word both ways in adjacent items**: `doubt` is wrong in one and the only right answer in the next, each pointing at the other. **Three labels resolved as a set** (`US-185`, `US-229`, `US-230`), finishing a class open since `gram.copula`. `gram.tense-agreement` now names what is true of **all six** producers — and worth recording: I briefed three producers, all in `be.js`, and **that was already stale**, since `past-simple.js` had added three more that day pointing the opposite way. `US-229` was decided as **one row, not two** — the reverse of the previous wave's call, and for a stated reason: `US-224` split because a label was **false** of a case, while here both would be true, so the deciding factor is **ranking, not honesty** (two rows of one producer each means a learner with the habit plausibly sees neither, and both are the absence of the same word from the same cause — Telugu borrows no word to ask or to negate). `US-230` stays split because its halves are opposites and one wording could only say "the inversion is in the wrong place". **Two blobstore judgements, both against the tidier option.** `removeForPrompt` of an unknown prompt keeps returning `ok: true` and only stops *claiming* a deletion (`US-227`): it names a **state** that holds when it returns, and reporting failure would force every caller to special-case one failure code as success, teaching callers to ignore `ok`. Stranded rows are **counted and reported, never repaired** (`US-228`) — repair is self-defeating rather than merely impure, because a repaired row is unflagged and epoch-dated, i.e. the oldest unprotected row, so the next eviction takes it; repair is deletion with a fabricated date on the way. Deleting fails `BR-7`, and the likeliest author of such a row is this module before Wave 16's fix, so the likeliest content is the learner's own. **Four incidental defects fixed:** `.recording-status.info` had been written since `US-701` and never styled; the **microphone stream could stay hot**, because the old handler stopped tracks only on its own click path; an object URL was created for a 0-byte blob with *"Recording saved!"* printed over it; and the Record/Stop buttons were only re-enabled from the Stop click. **Process, and a correction to my own diagnosis.** Three content agents died having written nothing. My earlier finding — that they read too long before writing — was **wrong**: both deaths were at the *moment of writing*, because these files are 42–70KB and a single write that large times out. The §7 instruction ("write a complete file first, then refine") was itself the cause, so it is **amended** rather than added to: write a terse but structurally complete file, then enrich section by section. **Stale claims corrected here:** §3.5 items 4 and 5 both **resolved** — the workflow comment no longer quotes a test count (it now says to run `npm test`), and the "keep the recording" claim is met; §3.5 item 3 is unchanged and still half-stale, its citation rotted a second time (`app.js:2381-2389` → `:2391-2397`); §3.1's voice-recording row; §6.x's dated-observations note and V9 (ten points register); §6.y A17; §6.z's "no comparison against the model and no archive"; §6.aa rule 5 and its verdict block; **and — the largest correction of the wave — nine rows recording work as not-done which had shipped up to sixteen waves earlier**: §6's phase table read `☐` for phases 1–7 and 9, §3.5 item 2 (the `StorageManager` docs) was fixed by W4 in Wave 1, and W4/W5/W6/W8/W11 in the loose-wins table were all `☐` against work landed in Waves 1–3. That is the same contradiction B3 was kept visible for, and it is worse in §5 and the loose-wins table than anywhere else, because a reader triaging them would have spent a sitting on items that were already done. The cause is structural — waves get written into §2 and §8 without sweeping the index sections those entries invalidate — so the sweep now explicitly includes §6's phase table and the loose-wins table, not only §3.5 and §5. **Two new untrue claims recorded rather than fixed**, since this pass may not touch source, and both understate the project: `CURRICULUM.md:284` says grammar is *"3 points authored of the 24, of which 2 are reachable"* and that `be.js` is not in `index.html` (it is, at `:721`, and ten points register), and `CURRICULUM.md:295` says `topCategories()` is never rendered (it has been since Wave 13). **Still wrong, stated plainly:** `US-138` — the archive is **not** in `FR-DATA-4`'s export, verified, so a learner who exports their data does not get their recordings; `US-406` (duration comparison) is unblocked by the recorder and unbuilt, and `FR-SPK-3`'s free-production surface still does not exist; `lsn.gist` still has no producer; `US-110`/`OQ-7` remains the last learner-facing honesty defect; and the CI test job is still commented out. **One new decision needed: B12** — `CURRICULUM.md §3` Strand B lists 24 points and **none is register or audience**, so `register.js` took `syllabusNumber: 25`, documented as a reported gap rather than squatting on a named point. |
